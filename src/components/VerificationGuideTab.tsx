...
const INITIAL_TEST_CASES: TestCase[] = [
  ...
  {
    id: 'TC-09',
    title: 'DevLog AI: Structured Engineering Log Creation & Persistence',
    directive: 'Directive 3 & 6',
    scope: '6-field capture (Title, Work, Challenges, Learnings, Tech, Impact) saved to /users/{userId}/engineeringLogs',
    steps: [
      'Navigate to the "Engineering Logs" tab.',
      'Fill in Title, Work Performed, Challenges Encountered, Learnings, and Impact/Outcome.',
      'Add technology tags (e.g. TypeScript, Cloud Run, Firestore) or select quick-add pills.',
      'Click "Save Engineering Log" button (id: btn-save-work-log).',
      'Verify green confirmation banner displaying document path in /users/{uid}/engineeringLogs/{id}.',
      'Verify live card appears in "Your Persisted Work History" with technology badges and full metadata.',
    ],
    expected: 'Entry is sanitized with stripUndefined and written to owner-bound Firestore subcollection with zero insecure defaults.',
    status: 'PASS',
  },
  {
    id: 'TC-10',
    title: 'Backward Compatibility: Baseline Journal Entry Support',
    directive: 'Directive 3 & 6',
    scope: 'Preserving baseline journal entries in /users/{userId}/journalEntries alongside new engineering logs',
    steps: [
      'In the "Engineering Logs" tab, click the "Baseline Journal Entry" toggle (id: btn-switch-journal).',
      'Enter a Title and Journal Reflection Content in the legacy format.',
      'Click "Save Journal Entry".',
      'Verify live list updates and displays the entry with the "JOURNAL ENTRY" badge.',
      'Test filter pills between "All", "Engineering", and "Journal" to verify separate streams.',
    ],
    expected: 'Existing and new baseline journal entries continue working seamlessly alongside engineering logs.',
    status: 'PASS',
  },
  {
    id: 'TC-11',
    title: 'AI Standup Generator: Time Windows, Synthesis, & Persistence',
    directive: 'Task 3 & Directive 2, 3, 6',
    scope: 'Period selection (24h, 3d, 7d), Gemini synthesis (Yesterday, Today, Blockers, Achievements), clipboard copy, and owner-bound Firestore persistence',
    steps: [
      'Navigate to the "Standup Generator" tab (id: tab-standup_generator).',
      'Toggle between "Last 24 Hours", "Last 3 Days", and "Last 7 Days" (id: period-btn-24h, 3d, 7d).',
      'Inspect the Sourced Data metrics showing the count of user-isolated logs and achievements in that window.',
      'Click "Generate Standup with Gemini" (id: btn-generate-standup).',
      'Verify the loading spinner and fallback ladder status text during synthesis.',
      'Verify the generated standup displays the 4 required sections: Yesterday, Today, Blockers, Achievements.',
      'Click "Copy to Clipboard" (id: btn-copy-standup) and verify the "Copied!" feedback state.',
      'Click "Save to Firestore" (id: btn-save-standup) and verify persistence to /users/{userId}/standups/{standupId}.',
      'Verify the new record appears in the "Persisted Standup History" section with period tag and deletion control.',
    ],
    expected: 'Generates structured 4-section standup using Gemini fallback ladder with zero client-side keys and full owner-bound Firestore isolation.',
    status: 'PASS',
  },
  {
    id: 'TC-12',
    title: 'AI Sprint Summary Generator: Custom Windows, 8-Section Report, & Isolation',
    directive: 'Task 4 & Directive 2, 3, 6',
    scope: 'Sprint windows (7d, 14d, 30d, custom date range), 8-section report generation, copy, and owner-bound Firestore persistence (/users/{userId}/sprintSummaries/{summaryId})',
    steps: [
      'Navigate to the "Sprint Summary" tab (id: tab-sprint_summary).',
      'Select a sprint window: "Last 7 Days", "Last 14 Days", "Last 30 Days", or "Custom Date Range" (id: sprint-btn-7d, 14d, 30d, custom).',
      'If Custom Date Range is selected, adjust the Start Date and End Date input controls.',
      'Inspect the Sourced Data metrics showing the count of user-isolated logs, achievements, and standups in that window.',
      'Click "Generate Sprint Summary with Gemini" (id: btn-generate-sprint-summary).',
      'Verify the loading spinner and fallback ladder status during synthesis.',
      'Verify the generated report contains all 8 required sections: Sprint Overview, Major Deliverables, Technical Achievements, Business Impact, Challenges Encountered, Lessons Learned, Areas of Growth, and Suggested Next Priorities.',
      'Click "Copy Summary" (id: btn-copy-sprint-summary) and confirm "Copied!" feedback.',
      'Click "Save Summary" (id: btn-save-sprint-summary) and verify persistence to /users/{userId}/sprintSummaries.',
      'Confirm the persisted sprint record appears in the "Persisted Sprint Summaries" section with copy and deletion controls.',
    ],
    expected: 'Generates an executive 8-section sprint review from owner-bound logs, achievements, and standups via server-side Gemini ladder with full Firestore isolation.',
    status: 'PASS',
  },
];
